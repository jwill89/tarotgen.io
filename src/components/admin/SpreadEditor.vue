...
const props = withDefaults(
    defineProps<{ spread: Spread | null; saveLabel?: string }>(),
    { saveLabel: 'Save Spread' }
)
const emit = defineEmits<{
    save: [payload: { name: string; description: string; card_count: number; positions: SpreadPosition[] }]
    cancel: []
}>()
...
const slots = reactive<EditorSlot[]>([])
...
defineExpose({
    finishSaving: () => { saving.value = false },
    setError: (msg: string) => { saving.value = false; error.value = msg },
})
</script>
...
<template>
    <div>
        <div class="columns">
            <div class="column is-8">
                <div class="field">
                    ...
                    <input class="input" v-model="name" placeholder="e.g. Celtic Cross" />
                </div>
            </div>
            <div class="column is-4">
                <div class="field">
                    <label class="label">Number of Cards</label>
                    <input class="input" type="number" v-model.number="cardCount" min="1" max="78" />
                </div>
            </div>
        </div>
                ...
                >
                    <template v-for="i in placedIndexes" :key="'card-' + i">
                        ...
                        >
                            ...
                            <span class="editor-card-title" v-if="slots[i].title">{{ slots[i].title }}</span>
                        </div>
        ...
        >
            <div class="field" v-if="editIndex !== null">
