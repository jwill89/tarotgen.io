...
export function useLayoutTools(
    ...
    const setSelection = (indices: number[]): void => { selected.value = new Set(indices) }
    const clearSelection = (): void => { selected.value = new Set() }
    ...
    function align(edge: AlignEdge): void {
        ...
        switch (edge) {
            case 'left':    { const v = snapVal(Math.min(...xs)); idx.forEach(i => { slots[i].x = v }); break }
            case 'right':   { const v = snapVal(Math.max(...xs)); idx.forEach(i => { slots[i].x = v }); break }
            case 'hcenter': { const v = round2((Math.min(...xs) + Math.max(...xs)) / 2); idx.forEach(i => { slots[i].x = v }); break }
            case 'top':     { const v = snapVal(Math.min(...ys)); idx.forEach(i => { slots[i].y = v }); break }
            case 'bottom':  { const v = snapVal(Math.max(...ys)); idx.forEach(i => { slots[i].y = v }); break }
            case 'vmiddle': { const v = round2((Math.min(...ys) + Math.max(...ys)) / 2); idx.forEach(i => { slots[i].y = v }); break }
        }
    }
