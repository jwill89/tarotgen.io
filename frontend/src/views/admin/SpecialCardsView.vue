...
async function saveCard() {
  ...
  try {
    const result = isNew.value
      ? await api.post(
          endpoints.admin.decks.specialCards(editing.value.deck_id),
          editing.value,
          'Special card created.',
        )
      : await api.put(
          endpoints.admin.decks.specialCardById(editing.value.deck_id, editing.value.card_id),
          editing.value,
          'Special card updated.',
        )
    if (!result) return
    await fetchData()
    closeEdit()
...
async function deleteCard(card: SpecialCard) {
  ...
  const result = await api.del(
    endpoints.admin.decks.specialCardById(card.deck_id, card.card_id),
    'Special card deleted.',
  )
  ...
  >
    <template v-if="editing">
      ...
      <div class="field">
        ...
        <input v-model="editing.keywords" class="input" placeholder="comma, separated, keywords" />
      </div>
      <div class="field">
        <label class="label">Meaning</label>
        <textarea v-model="editing.meaning" class="textarea" rows="3"></textarea>
      </div>
      <div class="field">
        <label class="label">Advice</label>
        <textarea v-model="editing.advice" class="textarea" rows="3"></textarea>
      </div>
      <div class="field">
        <label class="label">Keywords (Reversed)</label>
        <input
          v-model="editing.keywords_reversed"
          class="input"
      ...
      <div class="field">
        ...
        <textarea v-model="editing.meaning_reversed" class="textarea" rows="3"></textarea>
      </div>
      <div class="field">
        <label class="label">Advice (Reversed)</label>
        <textarea v-model="editing.advice_reversed" class="textarea" rows="3"></textarea>
      </div>
    </template>
