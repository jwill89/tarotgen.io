...
watch(decks, (val) => {
    if (readingInfo.value && !deck.value) {
        deck.value = val.find(d => d.deck_id === readingInfo.value!.deck_id) ?? null
    }
}, { immediate: true })
...
function cardThumb(cardId: number): string {
    ...
    return '/assets/decks/' + deck.value.deck_id + '/thumbs/' + filename + '.webp'
}
...
watch(editIndex, (val, old) => {
    if (val !== null) suspend()
    else if (old !== null) { resume(); record() }
})
...
<template>
        ...
        <div class="container">
            ...
            <div class="field">
                ...
                <div class="spread-token-tray">
                    ...
                    >
                        <span>{{ i + 1 }}</span>
                        <span class="fdp-tray-name">{{ slots[i].cardName.split(' ').slice(0, 2).join(' ') }}</span>
                    </button>
    ...
    <BaseModal
        ...
        :title="editIndex !== null ? 'Position #' + (editIndex + 1) + ' — ' + slots[editIndex!].cardName : ''"
        max-width="26rem"
    ...
    >
        <template v-if="editingSlot">
            <div class="field">
                ...
                <input class="input" v-model="editingSlot.title" placeholder="e.g. The Present" autocomplete="off" />
            </div>
        </template>
        <template #footer>
    ...
    >
        <p class="mb-4">Your reading has been saved. Would you like to also save this card layout as a reusable spread?</p>

        <div class="field">
            <label class="label">Spread Name</label>
            <input class="input" v-model="spreadName" placeholder="e.g. My Three-Card Pull" maxlength="100" autocomplete="off" />
        </div>

        <div class="field">
            <label class="label">Description <span class="has-text-grey is-size-7 has-text-weight-normal">(optional)</span></label>
            <textarea class="textarea" v-model="spreadDescription" rows="3" placeholder="Describe the spread layout..."></textarea>
        </div>
        ...
        <div class="field" v-if="isLoggedIn">
            ...
            <p class="help" v-if="spreadSaveMode === 'personal'">Saved to your personal collection.</p>
            <p class="help" v-else>Submitted for review to become a public spread.</p>
        </div>
