...
function fakeResponse(body: unknown, { ok = true, status = 200 } = {}): Response {
    return { ok, status, json: async () => body, clone: () => ({ json: async () => body }) } as unknown as Response
}
...
describe('Contact form submission', () => {
    ...
    it('returns an error message on a non-ok response', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
            fakeResponse({ error: 'Name, email, and message are all required.' }, { ok: false, status: 400 }),
        ))
    ...
    it('detects rate limiting (429)', async () => {
        vi.stubGlobal('fetch', vi.fn().mockResolvedValue(
            fakeResponse({ error: 'Too many' }, { ok: false, status: 429 }),
        ))
